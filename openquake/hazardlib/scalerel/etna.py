...
class EtnaMSR(BaseASR, BaseMSR):
    ...
    of 1.5 (lengh:width). In order to preserve the rupture length, the
    same aspect ratio must be used in later calculations.
    ...
    def get_median_mag(self, area, rake):
        """
        This ASR returns magnitude (Ml) given the area (km^2); assumes
        aspect ratio=1; rake and standard deviation are ignored.
    ...
    def get_median_area(self, mag, rake):
        """
        This MSR return area (km^2) given the magnitude (Ml); assumes
        aspect ratio=1; rake and standard deviation are ignored.

        :param magnitude:
            Magnitude (Ml)
        """
        return 0.666666667*((10 ** (-1.633 + 0.482 * mag))**2)
